import os
import pandas as pd
from ApsSensorFault.logging import log  
from ApsSensorFault.entity.artifact_entity import ModelTrainerArtifact, DataValidationArtifact, ModelEvaluationArtifact
from ApsSensorFault.entity.config_entity import ModelEvaluationConfig
from ApsSensorFault.ml.metric.classification_metric import get_classification_score
from ApsSensorFault.utils.main_util import load_object, write_yaml_file
from ApsSensorFault.ml.model.estimator import ModelResolver, TargetValueMapping
from ApsSensorFault.constant.training_pipeline import TARGET_COLUMN


class ModelEvaluation:
    def __init__(self, data_validation_artifact: DataValidationArtifact,
                 model_trainer_artifact: ModelTrainerArtifact,
                 model_evaluation_config: ModelEvaluationConfig):
        try:
            self.data_validation_artifact = data_validation_artifact
            self.model_trainer_artifact = model_trainer_artifact
            self.model_evaluation_config = model_evaluation_config

        except Exception as e:
            log.exception(e)
            raise e


    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            valid_train_file = self.data_validation_artifact.valid_train_file_path
            valid_test_file = self.data_validation_artifact.valid_test_file_path

            train_df = pd.read_csv(valid_train_file)
            test_df = pd.read_csv(valid_test_file)

            df = pd.concat([train_df, test_df])
            y_true = df[TARGET_COLUMN]
            y_true.replace(TargetValueMapping().to_dict(), inplace=True)
            df.drop(TARGET_COLUMN, axis=1, inplace=True)

            train_model_file_path = self.model_trainer_artifact.trained_model_file_path
            model_reslover = ModelResolver()
            is_model_accepted = True

            if not model_reslover.is_model_exist():
                model_evaluation_artifact = ModelEvaluationArtifact(
                    is_model_accepted=is_model_accepted,
                    improved_accuracy=True,
                    best_model_path=None,
                    trained_model_path=train_model_file_path,
                    trained_model_metric_artifact=self.model_trainer_artifact.test_metric_artifact,
                    best_model_metric_artifact=None)
                
                return model_evaluation_artifact

            latest_model_path = model_reslover.get_best_model_path()
            latest_model = load_object(latest_model_path)
            traine_model = load_object(train_model_file_path)

            # y_true = df[TARGET_COLUMN]
            y_trained_pred = traine_model.predict(df)
            y_latest_model = latest_model.predict(df)

            trained_metric = get_classification_score(y_true, y_trained_pred)
            latest_metric = get_classification_score(y_true, y_latest_model)

            # improved_accuracy = trained_metric-latest_metric
            improved_accuracy = trained_metric.f1_score-latest_metric.f1_score

            if self.model_evaluation_config.change_threshold < improved_accuracy:
                # 0.02 < 0.03
                is_model_accepted = True
            else:
                is_model_accepted = False

            model_evaluation_artifact = ModelEvaluationArtifact(
                is_model_accepted=is_model_accepted,
                improved_accuracy=improved_accuracy,
                best_model_path=latest_model_path,
                trained_model_path=train_model_file_path,
                trained_model_metric_artifact=trained_metric,
                best_model_metric_artifact=latest_metric 
                )

            model_eval_report = model_evaluation_artifact.__dict__
            write_yaml_file(file_path=self.model_evaluation_config.report_file_path, content=model_eval_report)
            return model_evaluation_artifact


        except Exception as e:
            log.exception(e)
            raise e